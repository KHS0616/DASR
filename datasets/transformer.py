...
def makeTransform(opt):
    compose_list = []
    if opt["hflip"]:
        compose_list.append(RandomHorizontalFlip())
    if opt["vflip"]:
        compose_list.append(RandomVerticalFlip())
    if opt["rot"]:
        compose_list.append(RandomRotation(90))
    if opt["crop"]["status"]:
        compose_list.append(RandomCrop(opt["crop"]["size"]))
    if opt["resize"]["status"]:
        compose_list.append(Resize(opt["resize"]["size"]))
    if opt["tensor"]:
